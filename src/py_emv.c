...
PyMODINIT_FUNC initemv(void)
{
	...
	_INT_CONST(m, DATA_BINARY);
	_INT_CONST(m, DATA_TEXT);
	_INT_CONST(m, DATA_INT);
	_INT_CONST(m, DATA_BCD);
	_INT_CONST(m, DATA_DATE);

	_INT_CONST(m, ERR_SYSTEM);
	_INT_CONST(m, ERR_CCID);
	_INT_CONST(m, ERR_ICC);
	_INT_CONST(m, ERR_EMV);

	_INT_CONST(m, ERR_SUCCESS);
	_INT_CONST(m, ERR_DATA_ELEMENT_NOT_FOUND);
	_INT_CONST(m, ERR_BAD_PIN_FORMAT);
	_INT_CONST(m, ERR_FUNC_NOT_SUPPORTED);
	_INT_CONST(m, ERR_KEY_NOT_FOUND);
	_INT_CONST(m, ERR_KEY_SIZE_MISMATCH);
	_INT_CONST(m, ERR_RSA_RECOVERY);
	_INT_CONST(m, ERR_CERTIFICATE);
	_INT_CONST(m, ERR_SSA_SIGNATURE);
	_INT_CONST(m, ERR_BAD_PIN);

	_INT_CONST(m, TAG_MAGSTRIP_TRACK2);
	_INT_CONST(m, TAG_PAN);
	_INT_CONST(m, TAG_RECORD);
	_INT_CONST(m, TAG_CDOL1);
	_INT_CONST(m, TAG_CDOL2);
	_INT_CONST(m, TAG_CVM_LIST);
	_INT_CONST(m, TAG_CA_PK_INDEX);
	_INT_CONST(m, TAG_ISS_PK_CERT);
	_INT_CONST(m, TAG_ISS_PK_R);
	_INT_CONST(m, TAG_SSA_DATA);
	_INT_CONST(m, TAG_CARDHOLDER_NAME);
	_INT_CONST(m, TAG_DATE_EXP);
	_INT_CONST(m, TAG_DATE_EFF);
	_INT_CONST(m, TAG_ISSUER_COUNTRY);
	_INT_CONST(m, TAG_SERVICE_CODE);
	_INT_CONST(m, TAG_PAN_SEQ);
	_INT_CONST(m, TAG_USAGE_CONTROL);
	_INT_CONST(m, TAG_APP_VER);
	_INT_CONST(m, TAG_IAC_DEFAULT);
	_INT_CONST(m, TAG_IAC_DENY);
	_INT_CONST(m, TAG_IAC_ONLINE);
	_INT_CONST(m, TAG_MAGSTRIP_TRACK1);
	_INT_CONST(m, TAG_ISS_PK_EXP);
	_INT_CONST(m, TAG_SDA_TAG_LIST);
	_INT_CONST(m, TAG_UNPREDICTABLE_NUMBER);

	_INT_CONST(m, AC_AAC);
	_INT_CONST(m, AC_TC);
	_INT_CONST(m, AC_ARQC);
	_INT_CONST(m, AC_CDA);

	_INT_CONST(m, AIP_CDA);
	_INT_CONST(m, AIP_ISS);
	_INT_CONST(m, AIP_TRM);
	_INT_CONST(m, AIP_CVM);
	_INT_CONST(m, AIP_DDA);
	_INT_CONST(m, AIP_SDA);

	_INT_CONST(m, AUC1_DOMESTIC_CASH);
	_INT_CONST(m, AUC1_INT_CASH);
	_INT_CONST(m, AUC1_DOMESTIC_GOODS);
	_INT_CONST(m, AUC1_INT_GOODS);
	_INT_CONST(m, AUC1_DOMESTIC_SERVICES);
	_INT_CONST(m, AUC1_INT_SERVICES);
	_INT_CONST(m, AUC1_ATM);
	_INT_CONST(m, AUC1_NON_ATM_TERMINALS);
	_INT_CONST(m, AUC2_DOMESTIC_CASHBACK);
	_INT_CONST(m, AUC2_INT_CASHBACK);
